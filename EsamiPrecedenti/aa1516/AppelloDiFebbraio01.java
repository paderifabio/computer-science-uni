...
public class AppelloDiFebbraio01 {
	 ...
	 *          per ogni i,
	 *          r[i] e' true se gli elementi di posizione i in 
	 *          a e b differiscono di una unita' o due unita' (non usare le librerie).
	 * Si osservi che i parametri attuali possono avere lunghezze differenti.
	 * Inoltre, exUno non deve generare alcun tipo d'errore.
	 *
	 * ESEMPI. Se a -> {1,2,3,4}, b -> {2,5,1,9,10} allora r -> {true,false,true,false}
	 *
	 * DOVE SCRIVERE exUno. 
	 ...
	 * c) exDueRic: 
	 *    1) restituisce un valore intero ottenuto come somma di tutti gli elementi di a con posizione
	 *       pari per i quali il metodo test sia vero. Se nessun elemento soddisfa il test
	 *       exDueRic restituisce 0.
	 *    2) ad ogni chiamata ricorsiva, exDueRic procede nella visita dell'array a, suddividendola in due 
	 *       parti la cui lunghezza differisca al piu' di una unita'.
	 *
	 * La chiamata a exDue non deve generare alcun tipo d'errore.
	 * 
	 * ESEMPI.
	 * Se b->{10, 2, 5, 4, 1, 2} allora exDue(b) restituisce 6 ottenuto dalla somma 5+1. 
	 * 
	 * DOVE SCRIVERE exDue. 
	 ...
	 */
	   ...
	   *         (n pari && exTre(n) < 1) || (n dispari && exTre(n) > 0)
